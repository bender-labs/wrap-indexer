...
export class TezosFinalizedWrapIndexer {
  ...
  async index(): Promise<void> {
    const minLevelToCheck = (await this._getNetworkLevel()) - this._tezosConfiguration.confirmationsThreshold;
    const mintsBigMap = await this._getMintsBigMap();
    const erc20Wraps = await this._wrapDao.getNotFinalizedERC20();
    erc20Wraps.concat(await this._wrapDao.getFinalizedERC20UntilLevel(minLevelToCheck))
    ...
    erc721Wraps.concat(await this._wrapDao.getFinalizedERC721UntilLevel(minLevelToCheck))
  ...
  private async _isInMintsMap(bigMap: any, blockHash: string, logIndex: number): Promise<boolean | null> {
    try {
  ...
  private async _updateWrapState(wrap: ERC20Wrap | ERC721Wrap, minted: boolean): Promise<void> {
    let transaction;
    try {
      if (minted && wrap.status === 'asked') {
        transaction = await this._dbClient.transaction();
        await this._wrapDao.setStatus(wrap, 'finalized', await this._getNetworkLevel(), transaction);
