...
export class EthereumQuorumIndexer {
  ...
  constructor(logger: Logger, ethereumConfiguration: EthereumConfig, ethereumProvider: ethers.providers.Provider, dbClient: Knex) {
    this._logger = logger;
  ...
  async index(): Promise<void> {
    ...
    let transaction;
    try {
      const contract = new ethers.Contract(this._ethereumConfiguration.wrapContractAddress, this._ethereumConfiguration.wrapABI, this._ethereumProvider);
