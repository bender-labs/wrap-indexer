...
export class ErcWrapDAO {
  ...
  async getERC20ByTransactionHash(
  ...
  async getERC721ByTransactionHash(
    transactionHash: string
  ): Promise<ERC721Wrap[]> {
    return this._dbClient
      .table<ERC721Wrap>('erc721_wraps')
  ...
  private _table(unwrap: ERC20Wrap | ERC721Wrap): string {
    return (unwrap as ERC20Wrap).amount !== undefined
      ? 'erc20_wraps'
      : 'erc721_wraps';
  }
