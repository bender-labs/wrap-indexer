...
import { ERC20Unwrap, ERC721Unwrap, ERCUnwrapStatus } from '../domain/ERCUnwrap';
...
export class ErcUnwrapDAO {
  ...
  async setStatus(unwrap: ERC20Unwrap | ERC721Unwrap, status: ERCUnwrapStatus, level: number, transaction: Knex.Transaction): Promise<void> {
    await this._dbClient.table(this._table(unwrap)).transacting(transaction).where({ id: unwrap.id }).update({ status, finalizedAtLevel: level });
  }
  ...
  async getFinalizedERC20UntilLevel(level: number): Promise<ERC20Unwrap[]> {
    return this._dbClient.table<ERC20Unwrap>('erc20_unwraps').where({ status: 'finalized' }).andWhere('finalized_at_level', '>=', level);
  }
  ...
  async getFinalizedERC721UntilLevel(level: number): Promise<ERC721Unwrap[]> {
    return this._dbClient.table<ERC721Unwrap>('erc721_unwraps').where({ status: 'finalized' }).andWhere('finalized_at_level', '>=', level);
  }

  async getERC20ByOperationHash(operationHash: string): Promise<ERC20Unwrap[]> {
    return this._dbClient.table<ERC20Unwrap>('erc20_unwraps').where({ operationHash });
  }

  async getERC721ByOperationHash(operationHash: string): Promise<ERC721Unwrap[]> {
    return this._dbClient.table<ERC721Unwrap>('erc721_unwraps').where({ operationHash });
  }
  ...
  async remove(unwraps: (ERC20Unwrap | ERC721Unwrap)[], transaction: Knex.Transaction): Promise<void> {
    for (const unwrap of unwraps) {
      await this._dbClient.table(this._table(unwrap)).transacting(transaction).where({ id: unwrap.id }).delete();
    }
  }
