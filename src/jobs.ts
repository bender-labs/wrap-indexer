...
export function scheduleJobs(dependencies: Dependencies): Crontab {
  ...
  crontab.register(() => new TezosInitialUnwrapIndexer(dependencies).index(), everyMinute);
  crontab.register(() => new SignatureIndexer(dependencies).index(), everyMinute);
  crontab.register(() => new EthereumFinalizedUnwrapIndexer(dependencies).index(), everyMinute);
  crontab.register(() => new TezosFinalizedWrapIndexer(dependencies).index(), everyMinute);
