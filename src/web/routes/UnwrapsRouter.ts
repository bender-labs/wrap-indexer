...
function build({ dbClient, tezosConfiguration, tezosToolkit }: Dependencies): Router {
  const router = Router();
  router.get('/', async (req: Request, res: Response) => {
    ...
    const query = new PendingUnwrapsQuery(dbClient, tezosConfiguration, tezosToolkit);
